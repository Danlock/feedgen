...
func (m *mangaStore) UpsertRelease(ctx context.Context, releases []scrape.MangaRelease) error {
	...
	// Unfortunately CockroachDB doesn't support the RowsAffected function
	_, err := m.db.ExecContext(ctx, releaseQuery, valuesArr...)
	if err != nil {
		logger.Errf(ctx, "Failed to upsert release with query %s and err: %+v", releaseQuery, ErrDetails(err))
		return errors.WithStack(err)
	}
	logger.Dbgf(ctx, "Upserted %d releases", len(releases)-releaesMissingMUIDs)
	return nil
}
